...
PYBIND11_MODULE(lumen_py, m) {
    ...
    py::class_<lumen::Buffer>(m, "Buffer")
        .def("data", [](lumen::Buffer &b) {
            ...
            for (auto d : b.shape()) py_shape.push_back((ssize_t)d);
            
            // keep_alive<0, 1> ensures the Buffer stays alive as long as this numpy array exists
