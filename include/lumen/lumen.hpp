...
public:
    ...
    virtual void free_buffer(void* device_ptr, size_t size) = 0; // Signature requires size
    virtual void execute(const std::string& op_name, const std::vector<Buffer*>& inputs, Buffer* output) = 0;
    virtual std::shared_ptr<Event> sync(std::vector<QueuedOp>& queue) = 0; // Returns Event
protected:
    MemoryPool pool_;
};
